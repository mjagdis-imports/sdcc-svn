...
struct expr *esp;
{
  ...
  if ((c = getnb()) == '#') 
  ...
  else 
    {
      ...
      else if ((indx = admode(R8U1)) != 0) 
        {
          mode = S_R8U1;
        } 
      else if ((indx = admode(R8U2)) != 0) 
        {
          mode = S_R8U2;
        } 
#endif
      ...
      if ((c = getnb()) == LFIND) 
        {
          // TLCS-90 can handle sp as index reg.. (indx & 0xFF) == HL) experimental for HL+A
          if ((indx = admode(R16)) != 0 && ((indx & 0xFF) == IX || (indx & 0xFF) == IY || (indx & 0xFF) == SP  || (indx & 0xFF) == HL))
            {
