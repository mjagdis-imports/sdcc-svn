...
static bool XAinst_ok(const assignment &a, unsigned short int i, const G_t &G, const I_t &I)
{
  ...
  if(ic->op == IPUSH && (unused_A || dying_A || left_in_A || operand_in_reg(left, REG_Y, ia, i, G) || left_in_X))
    return(true);
  ...
  // Y must be free
  if ((ic->op == GET_VALUE_AT_ADDRESS) && (unused_Y || dying_Y))
    return true;
  if (POINTER_SET(ic) && (unused_Y || dying_Y))
    return true;
  if (ic->op == '=')
    return true;
...
// This is just a dummy for now, it probably isn't really needed for m6502 due to the low number of registers.
template <class G_t, class I_t>
