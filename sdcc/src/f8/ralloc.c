...
/*-----------------------------------------------------------------*/
void
f8SpillThis (symbol *sym, bool force_spill)
{
  int i;

  D (D_ALLOC, ("f8SpillThis: spilling %p (%s)\n", sym, sym->name));

  /* if this is rematerializable or has a spillLocation
     we are okay, else we need to create a spillLocation
     for it */
  if (!(sym->remat || sym->usl.spillLoc) || (sym->usl.spillLoc && !sym->usl.spillLoc->onStack)) // f8 port currently only supports on-stack spill locations in code generation.
    createStackSpil (sym);
...
serialRegMark (eBBlock ** ebbs, int count)
{
  ...
  f8_call_stack_size = 2; // Saving of register to stack temporarily.
  ...
  for (i = 0; i < count; i++)
    {
      ...
      for (ic = ebbs[i]->sch; ic; ic = ic->next)
        {
          if ((ic->op == CALL || ic->op == PCALL) && ic->parmBytes + 5 > f8_call_stack_size)
            {
              sym_link *dtype = operandType (IC_LEFT (ic));
              sym_link *ftype = IS_FUNCPTR (dtype) ? dtype->next : dtype;

              /* 5 for saving all registers at call site + 2 for big return value */
              f8_call_stack_size = ic->parmBytes + 5 + 2 * (getSize (ftype->next) > 4);
            }
...
void
f8RegFix (eBBlock ** ebbs, int count)
{
  int i;
...
/*-----------------------------------------------------------------*/
void
f8_assignRegisters (ebbIndex *ebbi)
{
  ...
  f8_extend_stack = f8_call_stack_size > 255;

  /* Invoke optimal register allocator */
  ic = f8_ralloc2_cc (ebbi);

  /* redo the offsets for stacked automatic variables */
  if (currFunc && !f8_extend_stack && currFunc->stack + f8_call_stack_size > 255)
    {
      _G.slocNum = 0;
