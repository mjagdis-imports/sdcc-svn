...
extern "C"
{
  #include "ralloc.h"
  #include "gen.h"
  float dryF8iCode (iCode *ic);
  bool f8_assignment_optimal;
  long int f8_call_stack_size;
  ...
  for(boost::tie(oi, oi_end) = G[i].operands.equal_range(OP_SYMBOL_CONST(o)->key); oi != oi_end; ++oi)
    {
      ...
      if(a.global[v] >= 0)
        { 
          sym->regs[I[v].byte] = f8_regs + a.global[v];   
          sym->nRegs = I[v].size;
        }
      else
        {
...
{
  // f8 code generation is very flexible, and can handle nearly anything (including variables where some bytes are spilt, while others are not).
...
static void extra_ic_generated(iCode *ic)
{
  if(ic->op == '>' || ic->op == '<' || ic->op == LE_OP || ic->op == GE_OP || ic->op == EQ_OP || ic->op == NE_OP ||
    ic->op == BITWISEAND && (IS_OP_LITERAL (IC_LEFT (ic)) || IS_OP_LITERAL (IC_RIGHT (ic))) || ic->op == GETABIT || ic->op == GET_VALUE_AT_ADDRESS)
    {
      iCode *ifx;
      ...
      if (ic->op == GET_VALUE_AT_ADDRESS &&
        (getSize(operandType(IC_RESULT(ic))) > 2 || IS_BITVAR (getSpec (operandType (IC_RESULT(ic)))) && SPEC_BLEN (getSpec (operandType (IC_RESULT(ic)))) > 8)) // Similar for pointer read
        {
          return;
        }
      if (ic->op == GETABIT)
  ...
  for(unsigned int v = 0; v < boost::num_vertices(I); v++)
    {
      ...
      if(winner.global[v] >= 0)
        sym->regs[I[v].byte] = f8_regs + winner.global[v];   
      else
        {
          sym->regs[I[v].byte] = 0;
          spilt = true;
...
iCode *f8_ralloc2_cc(ebbIndex *ebbi)
{
  ...
  f8_assignment_optimal = !tree_dec_ralloc(tree_decomposition, control_flow_graph, conflict_graph, stack_conflict_graph);
