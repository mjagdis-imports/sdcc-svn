...
public:
  ...
  mkdeps ()
    : module_name (NULL), cmi_name (NULL), is_header_unit (false),
      quote_lwm (0), first_phony_dep (1)
  {
  }
  ~mkdeps ()
  {
...
/* Sets the default target if none has been given already.  An empty
   ...
   is quoted for MAKE.  */
void
deps_add_default_target (const class cpp_reader *pfile, class mkdeps *d, const char *tgt)
{
...
# define TARGET_OBJECT_SUFFIX ".o"
      ...
      // if obj_ext != ".o":
      //    fprintf(stderr, "custom objext %s, deprecate this?\n", obj_ext);

      char *o = (char *) alloca (strlen (start) + strlen (obj_ext) + 1);
